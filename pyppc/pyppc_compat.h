...
------------------------------------------------------------------------*/
/**
 * Python compability file for making it possible to compile baltrad-ppc for both python >2.6 and python3
 * @file
...
#ifndef PYPPCCOMPAT_H
#define PYPPCCOMPAT_H
...
#define PY_COMPARE_ATTRO_NAME_WITH_STRING(ptr, name) PyPpcAPI_CompareWithASCIIString(ptr, name)

#define PY_COMPARE_STRING_WITH_ATTRO_NAME(name, ptr) PyPpcAPI_CompareWithASCIIString(ptr, name)
 ...
 * @return 0, -1 or 1
 */
static int PyPpcAPI_CompareWithASCIIString(PyObject* ptr, const char* name) {
  int result = -1;
